...
function JsonImport() {
  ...
  const handleFile = (file) => {
    ...
    axios.post('/api/import-json/', formData, {
      headers: {
    ...
    .catch(error => {
      ...
      setMessage(error.response?.data?.detail || 'Error al importar el archivo');
      setMessageType('error');
    })
    .finally(() => {
      setIsLoading(false);
  ...
  return (
    <div className="mt-6 text-center">
      <h3 className="text-sm text-blue-300 mb-2">¿Tienes un respaldo? Restaura tus datos</h3>
      <div
        ...
        onDrop={handleDrop}
        onClick={isLoading ? null : openFileSelector}
      >
        <input 
          type="file" 
          ref={fileInputRef}
        ...
        {isLoading ? (
          ...
          <p className="text-sm">Arrastra un archivo JSON o haz clic para seleccionarlo</p>
        )}
      </div>
