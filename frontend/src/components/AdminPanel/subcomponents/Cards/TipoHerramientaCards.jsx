...
function TipoHerramientaCards({ items, onEdit, onDelete }) {
  const prevItemsRef = useRef(null);
  const [visibleItems, setVisibleItems] = useState([]);
  const [expandedId, setExpandedId] = useState(null);

  useEffect(() => {
    console.log("TipoHerramientaCards recibió items:", items);
    ...
    return (
      <div className="w-full flex justify-center items-center p-8 bg-gray-800/60 rounded-lg border border-gray-700">
        <p className="text-gray-400 text-lg">
          No hay tipos de herramientas registrados. Crea uno nuevo usando el botón
  ...
  return (
    <>
      {items.map((tipoHerramienta, index) => (
        <MemoizedTipoHerramientaCard
          key={tipoHerramienta.id}
          tipoHerramienta={tipoHerramienta}
          index={index}
          onEdit={handleEdit}
          onDelete={handleDelete}
          expanded={expandedId === tipoHerramienta.id}
          onExpand={() => handleExpand(tipoHerramienta.id)}
        />
      ))}
    </>
  );
...
function TipoHerramientaCard({ tipoHerramienta, index, onEdit, onDelete, expanded, onExpand }) {
  return (
    <div
      className={`w-full bg-gray-800/80 border border-gray-700 rounded-lg overflow-hidden shadow-lg transition-all duration-300 hover:shadow-purple-900/20 mb-4 ${
      ...
      <div className="p-4">
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            <div className="flex-shrink-0 h-14 w-14 rounded-full bg-gradient-to-br from-purple-600 to-indigo-600 flex items-center justify-center text-white font-bold text-xl">
              {tipoHerramienta.nombre ? tipoHerramienta.nombre.charAt(0).toUpperCase() : "H"}
            </div>
        ...
        <div className="mt-4 flex justify-end space-x-2 border-t border-gray-700 pt-3">
          <button
            ...
            onClick={() => onEdit(tipoHerramienta)}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5"
          ...
          <button
            ...
            title="Eliminar tipo de herramienta"
            onClick={() => onDelete(tipoHerramienta)}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5"
...
const MemoizedTipoHerramientaCard = memo(TipoHerramientaCard);

export default TipoHerramientaCards;