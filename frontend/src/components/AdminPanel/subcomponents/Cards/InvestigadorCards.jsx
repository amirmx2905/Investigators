  ...
  useEffect(() => {
    const fetchUsuariosAsignados = async () => {
      ...
      try {
        ...
        console.log('Investigadores con usuario asignado (cards):', idsConUsuario);
  ...
  if (cargandoUsuarios) {
    return (
      <div className="col-span-full text-center py-8 text-blue-400 flex flex-col items-center">
        <svg className="animate-spin h-8 w-8 mb-2 text-blue-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
function InvestigadorCard({ investigador, index, onEdit, onDelete, tieneUsuario }) {
  const [expanded, setExpanded] = useState(false);
  
  // Determinar nivel SNII con colores personalizados para cada nivel
  const getNivelSNII = () => {
  ...
  return (
    <div
      className={`bg-gray-800/80 border border-gray-700 rounded-lg overflow-hidden shadow-lg transition-all duration-300 hover:shadow-blue-900/20 ${
        expanded ? 'hover:border-blue-500/50 ring-1 ring-blue-500/20' : 'hover:border-blue-500/30'
      ...
      <div className="p-4">
        ...
        <div className="flex items-center mb-2">
          <div className="flex-shrink-0 h-12 w-12 rounded-full bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold text-xl relative">
            {investigador.nombre ? investigador.nombre.charAt(0).toUpperCase() : "I"}
            
            {/* Indicador de estado (activo/inactivo) */}
            <span className={`absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-gray-800 ${
              investigador.activo ? 'bg-green-500' : 'bg-red-500'
            }`} title={investigador.activo ? "Activo" : "Inactivo"}></span>
          </div>
          
          <div className="ml-3 flex-grow min-w-0">
            <h3 className="font-semibold text-white flex items-center space-x-2">
              <span className="truncate">{investigador.nombre}</span>
              <span className={`text-xs px-2 py-0.5 rounded-full border ${nivelSNII.color} whitespace-nowrap`}>
                {nivelSNII.badge}
              </span>
            </h3>
            <p className="text-sm text-gray-400 truncate">{investigador.correo}</p>
          </div>
        ...
        <div className="bg-gradient-to-r from-gray-800/70 to-gray-900/70 rounded-lg p-2 mb-3 flex items-center justify-between">
          <div className="text-sm text-gray-300 flex items-center">
            ...
            <span className="ml-1.5 font-medium text-gray-200">{investigador.id}</span>
            
            {tieneUsuario && (
              <span className="ml-2 text-blue-400 text-xs flex items-center" title="Tiene usuario asignado">
        ...
        <div className={`transition-all duration-300 ${
          ...
          <div className="grid grid-cols-2 gap-2 mt-2 text-sm">
            <div className="bg-gray-700/40 p-2 rounded">
              ...
              <span className="ml-1.5 text-gray-200 block truncate">{investigador.especialidad_nombre || "Sin especialidad"}</span>
            </div>
            <div className="bg-gray-700/40 p-2 rounded">
              <span className="text-gray-400">Área:</span>
              <span className="ml-1.5 text-gray-200 block truncate">{investigador.area_nombre || "Sin área"}</span>
            </div>
          </div>
        </div>

        {/* Contenido expandible */}
        <div className={`transition-all duration-300 overflow-hidden space-y-3 ${
          expanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'
        }`}>
          {/* Indicador de usuario */}
          <div className="bg-gradient-to-r py-2.5 px-3 rounded-md text-white text-sm flex items-center justify-between"
               style={{backgroundImage: `linear-gradient(to right, rgb(17, 24, 39, 0.8), rgb(31, 41, 55, 0.8))`}}>
            <div className="flex items-center">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="bg-gray-700/40 hover:bg-gray-700/50 rounded-md p-2.5 transition-colors duration-200 flex items-center justify-between w-full">
              ...
              <span className="text-gray-200 font-medium truncate max-w-[60%]">{investigador.area_nombre || "Sin área"}</span>
            </div>
            ...
            <div className="bg-gray-700/40 hover:bg-gray-700/50 rounded-md p-2.5 transition-colors duration-200 flex items-center justify-between w-full">
              ...
              <span className="text-gray-200 font-medium truncate max-w-[60%]">{investigador.especialidad_nombre || "Sin especialidad"}</span>
            </div>
            ...
            <div className="bg-gray-700/40 hover:bg-gray-700/50 rounded-md p-2.5 transition-colors duration-200 flex items-center justify-between w-full">
              ...
              <span className="text-gray-200 font-medium truncate max-w-[60%]">{investigador.celular || "No registrado"}</span>
            </div>
            ...
            <div className="bg-gray-700/40 hover:bg-gray-700/50 rounded-md p-2.5 transition-colors duration-200 flex items-center justify-between w-full">
              ...
              <span className="text-gray-200 font-medium truncate max-w-[60%]">{investigador.correo}</span>
            </div>
          </div>
        </div>
