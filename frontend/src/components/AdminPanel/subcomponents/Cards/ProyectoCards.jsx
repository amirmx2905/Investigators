...
function ProyectoCard({ proyecto, index, onEdit, onDelete }) {
  ...
  const formatDate = (dateString) => {
    ...
    // Usar el mismo formato que en la tabla
  ...
  // Función para formatear descripción similar a la tabla
  ...
  // Determinar qué campo de descripción usar (explicacion o descripcion)
  const descripcionProyecto = proyecto.descripcion || proyecto.explicacion || "";
        ...
        <div className="bg-gradient-to-r from-gray-800/70 to-gray-900/70 rounded-lg p-2 mb-3">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-gray-300 flex items-center">
              ...
              <span className="ml-1.5 font-medium text-gray-200">{proyecto.id}</span>
            </div>
            ...
            <div className="flex items-center mt-1.5 sm:mt-0">
              ...
              <span className="ml-1.5 text-gray-200 text-xs">{formatDate(proyecto.fecha_inicio)} → {formatDate(proyecto.fecha_fin)}</span>
            </div>
          </div>
        </div>
        ...
        <div className={`transition-all duration-300 ${
          ...
          <div className="grid grid-cols-2 gap-2 mt-2 text-sm">
            <div className="bg-gray-700/40 p-2 rounded">
              ...
              <span className="ml-1.5 text-gray-200 block truncate">{formatDate(proyecto.fecha_inicio)}</span>
            </div>
            <div className="bg-gray-700/40 p-2 rounded">
              <span className="text-gray-400">Fin:</span>
              <span className="ml-1.5 text-gray-200 block truncate">{formatDate(proyecto.fecha_fin)}</span>
            </div>
          </div>
        </div>
        ...
        <div className={`transition-all duration-300 overflow-hidden space-y-3 ${
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="bg-gray-700/40 hover:bg-gray-700/50 rounded-md p-2.5 transition-colors duration-200 flex items-center justify-between w-full">
              ...
              <span className="text-gray-200 font-medium truncate max-w-[60%]">{formatDate(proyecto.fecha_inicio)}</span>
            </div>
            ...
            <div className="bg-gray-700/40 hover:bg-gray-700/50 rounded-md p-2.5 transition-colors duration-200 flex items-center justify-between w-full">
              ...
              <span className="text-gray-200 font-medium truncate max-w-[60%]">{formatDate(proyecto.fecha_fin)}</span>
            </div>
            ...
            <div className="bg-gray-700/40 hover:bg-gray-700/50 rounded-md p-2.5 transition-colors duration-200 flex items-center justify-between w-full">
              ...
              <span className={`px-2 py-0.5 text-xs rounded-full ${getStatusStyles(proyecto.estado)}`}>
                {proyecto.estado || "Sin estado"}
              </span>
            </div>
            ...
            {descripcionProyecto && (
              <div className="bg-gray-700/40 hover:bg-gray-700/50 rounded-md p-2.5 transition-colors duration-200 w-full">
                ...
                <p className="text-gray-200 text-sm mt-1">{descripcionProyecto}</p>
              </div>
            )}
          </div>
        </div>
